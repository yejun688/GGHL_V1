...
label_path = '/data0/datasets/slam_parkingslot/new_data/newdata/'
store_path = '/data0/datasets/slam_parkingslot/new_data/Annotations/'
...
for label in label_list:
    ...
    with open(label_info_path, encoding="GBK") as label_info:  
        new_boxes = []
        new_boxes1 = []
    
        for box in label_info.readlines()[2:]:  # 第三行
            ...
            if box[9] != '2':
                ...
                with open(label_store_path, 'a') as file1:
                    file1.writelines(str(labelout))

        each_boxes = ("/data0/datasets/slam_parkingslot/new_data/image/" + label.split('.')[0] + 
                       '.png' + " " + " ".join(new_boxes) + "\n")
        temp = "/data0/datasets/slam_parkingslot/new_data/image/" + label.split('.')[0] + '.png' + " " + "\n"
        if each_boxes != temp:
            boxes.append(each_boxes)

    with open("/data0/GGHL/dataR/train.txt", 'a') as file:
